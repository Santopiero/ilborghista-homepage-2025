...
export function getCurrentUser() { return readFromLocalStorage(USER_KEY, null); }
export function setCurrentUser(user) { writeToLocalStorage(USER_KEY, user); }
export function logoutUser() { localStorage.removeItem(USER_KEY); }

// ============================================================================
// CREATORS
// ============================================================================
const CREATORS_KEY = "ib_creators";
export function listCreators() { return readFromLocalStorage(CREATORS_KEY, []); }
export function getCreator(id) { return listCreators().find(c => c.id === id) || null; }
export function addCreator(creator) {
  const creators = listCreators(); creators.push(creator);
  writeToLocalStorage(CREATORS_KEY, creators); return creator;
}
export function updateCreator(updated) {
  const creators = listCreators();
  const idx = creators.findIndex(c => c.id === updated.id);
  if (idx >= 0) { creators[idx] = updated; writeToLocalStorage(CREATORS_KEY, creators); return true; }
  return false;
}
export function getMyCreatorProfile() {
  const currentUser = getCurrentUser(); if (!currentUser) return null;
  const creators = listCreators(); return creators.find(c => c.userId === currentUser.id) || null;
}
...
export function listBorghi() { return readFromLocalStorage(BORGHI_KEY, []); }
export function getBorgo(slug) { return listBorghi().find(b => b.slug === slug) || null; }
export const findBorgoBySlug = getBorgo;
export function findBorgoByName(name) {
  const s = slugify(name);
  return listBorghi().find(b => b.slug === s || slugify(b.name) === s) || null;
}
...
export function listAttivita() { return readFromLocalStorage(ATTIVITA_KEY, []); }
export function getAttivita(id) { return listAttivita().find(p => p.id === id) || null; }
export function listAttivitaByBorgo(borgoSlug) { return listAttivita().filter(p => p.borgoSlug === borgoSlug); }
export function addAttivita(item) {
  const items = listAttivita(); items.push(item);
  writeToLocalStorage(ATTIVITA_KEY, items); return item;
}
export function updateAttivita(updated) {
  const items = listAttivita();
  const idx = items.findIndex(p => p.id === updated.id);
  if (idx >= 0) { items[idx] = updated; writeToLocalStorage(ATTIVITA_KEY, items); return true; }
  return false;
}
// alias compatibilità
