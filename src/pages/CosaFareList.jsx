...
export default function CosaFareList() {
  ...
  const items = useMemo(() => {
    return allItems.filter(i => {
      ...
      return matchQ && matchC && i.type === "cosa-fare";
    });
  }, [allItems, q, cat]);

  return (
    <main className="max-w-6xl mx-auto px-4 pt-14 pb-16">
      {/* Barra categorie sticky sotto TopBar */}
      ...
      {/* Filtri sticky (spostati sotto la barra pallotte) */}
      <div className="sticky top-[calc(56px+40px)] z-20 bg-white/90 backdrop-blur border-y">
        <div className="max-w-6xl mx-auto px-4 py-2 flex items-center gap-2 overflow-x-auto">
          ...
          <span className="text-sm text-gray-600 shrink-0">{items.length} risultati</span>
        </div>
      </div>
      ...
      <div className="mt-8 flex justify-center">
        <Link to={`/borghi/${slug}`} className="inline-flex items-center gap-1 text-sm px-3 py-2 rounded-xl border hover:bg-gray-50 transition">
